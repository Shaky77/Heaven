...
> 本件为对外回信草稿。链接已更正并通过 GitHub 工具更新（EN 钩子 KISS_Law-DSH → KISS-s_Law 基础版；原"实测 404"为下划线拼写误判）。
...
If you read its English twin — **KISS-s_Law** (github.com/Shaky77/KISS-s_Law) — alongside the Chinese repo (github.com/Shaky77/Weiwen-s_Law), you may land on the same realization I did: the plugin is the tip, and the causal structure underneath is far more general than a harness add-on. (The EN and CN repos are identical in content — just language counterparts, so use whichever you prefer.)
...
| 2 | 告诉对方唯稳律是什么；DSH 是武功秘籍/降维/非全部，可能偏差；**重点补「通用型」** | RSDHM 完整链 + 三公 + review不猜 + 内H不可侵 + 同构**收窄可测** + **通用型重点补充**（市面因果工具皆领域限定，唯稳律通用→故为因果律本身白箱化呈现）；DSH = martial-arts manual / dimensionally reduced / tip not whole；**EN 钩子 KISS-s_Law（基础版，与中文 Weiwen-s_Law 内容一致、互为参照）** | **说**：白箱可验证结构（全真、可独立复现）。**藏**：导图心法具体内容、活出来背景、多 agent 协作、真实商业化程度——只点到"deeper layer exists"留白 |
...
- **EN 钩子链接**：用户要求在「导图心法」处加 EN 仓链接做钩子。正确仓库名为连字符：`KISS-s_Law`（英文基础版）/ `Weiwen-s_Law`（中文基础版），二者内容一致、互为参照，均含框架导图，可直接打开使用。此前误以**下划线**拼写（`KISS_s_Law` / `Weiwen_s_Law`）实测才返回 404，并非仓库不存在；故钩子落 `github.com/Shaky77/KISS-s_Law`（英文基础版），与中文基础版 `Weiwen-s_Law` 成对，措辞"读它的英文孪生版，或许会有和我一样的感受"。
