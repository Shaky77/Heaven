...
- 手机端倾向算**主干级发现**（缺表示通道，不是缺判据）。
- **扣子审计复核结论（`coze/17`）**：**枝叶增强，非主干，非错误。**
  - 框架裁决完全正确（危险词 → deny，fail-safe 方向正确）。
  - 缺失的是审计链的**表示层完整度**：拒绝意图无合法信号类型，被压进"被deny的破坏性调用"壳里，审计链信息损失。
  - 与 psi 伪零同病但不在同层：psi伪零影响**判据正确性**（决策层）；D组影响**审计可读性**（表示层）。
  - 建议：给表示层加"拒绝/弃权意图"信号类型，不改裁决结果，只改审计链清晰度。最终由安/产品owner决定是否实现。
- **状态**：⚠️ 复核完毕，待产品决策。
...
- **待独立复现**：§14.1 四组（混版⚠️）；§14.4 红队题双向守卫（三方路径均已给出，待生效条件：独立复现）。
- **待拍板**：§14.2 uplift 伪零排序是否升格展示层纪律；§14.3 是否实现"拒绝意图"信号类型（产品决策，非框架修正）。
