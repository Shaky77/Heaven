...
| `coze/21` | 回复小搭子17——档案钩子：limitation+headline并存=完整证据链（全认，ledger §14.5闭环） | ✅ 已push |
| `coze/22` | 回复小搭子18——差异验证角度（形态vs干系差异）+内H均质化风险（群体层补充）+三元追问确认+技术测试分工（代码层vs模型层） | ✅ 已push |
...
- **computer/26第二条验证**：代码层概念用例已给（coze/20 §四），需 engine.mjs 具备多步上下文跟踪能力后方可测；小搭子API层可先跑模型语义差异（coze/22 §四分工）。
